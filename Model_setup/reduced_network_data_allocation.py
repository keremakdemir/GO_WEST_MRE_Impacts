...
all_area_names = [*df_full['Area Name'].unique()]
all_BA_names = [*df_full['NAME'].unique()]
...
for YY in Years:
    ...
    for NN in NODE_NUMBER:
        ...
        for UC in UC_TREATMENTS:
            ...
            for T_p in not_congested_line_scaling:
                
                for L_s in congested_line_scaling:
                
                    for BA_hurd in BA_hurdle_scaling:
            
                        path=str(Path.cwd().parent) + str(Path('/UCED/Simulation_folders/Exp' + str(NN) + UC + '_' + str(T_p) + '_' + str(L_s) + '_' + str(BA_hurd)  + '_' + str(YY)))
                        os.makedirs(path,exist_ok=True)
                        
                        path_2=str(Path.cwd().parent) + str(Path('/UCED/Simulation_folders/Exp' + str(NN) + UC + '_' + str(T_p) + '_' + str(L_s) + '_' + str(BA_hurd) + '_' + str(YY))) + str(Path('/Inputs'))
                        os.makedirs(path_2,exist_ok=True)
                        
                        path_3=str(Path.cwd().parent) + str(Path('/UCED/Simulation_folders/Exp' + str(NN) + UC + '_' + str(T_p) + '_' + str(L_s) + '_' + str(BA_hurd) + '_' + str(YY))) + str(Path('/Outputs'))
                        ...
                        df_WP = pd.read_csv('Model_inputs/nodal_wave.csv',header=0)
